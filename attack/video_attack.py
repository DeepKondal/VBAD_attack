...
def sim_rectification_vector(model, vid, tentative_directions, n, sigma, target_class, rank_transform, sub_num,
    ...
    with torch.no_grad():
        grads = torch.zeros(len(group_gen), device='cpu')  # Changed from 'cuda'
        count_in = 0
        loss_total = 0
        logging.info('sampling....')
        batch_loss = []
        ...
        for _ in range(n // sub_num):
            adv_vid_rs = vid.repeat((sub_num,) + (1,) * len(vid.size()))
            noise_list = torch.randn((sub_num // 2,) + grads.size(), device='cpu') * sigma  # Changed from 'cuda'
            ...
            if untargeted:
                ...
                loss = torch.nn.functional.cross_entropy(logits, torch.tensor(target_class, dtype=torch.long,
                                                                              device='cpu').repeat(sub_num),
                                                         reduction='none')  # Changed from 'cuda'
            batch_loss.append(loss)
            batch_idx.append(top_idx)
            batch_noise.append(all_noise)
        batch_noise = torch.cat(batch_noise, 0)
        ...
        if rank_transform:
            good_idx = torch.sum(batch_idx == target_class, 1).byte()
            changed_loss = torch.where(good_idx, batch_loss, torch.tensor(1000., device='cpu'))  # Changed from 'cuda'
            loss_order = torch.zeros(changed_loss.size(0), device='cpu')  # Changed from 'cuda'
            sort_index = changed_loss.sort()[1]
            loss_order[sort_index] = torch.arange(0, changed_loss.size(0), device='cpu', dtype=torch.float)  # Changed from 'cuda'
            available_number = torch.sum(good_idx).item()
            count_in += available_number
            unavailable_number = n - available_number
            unavailable_weight = torch.sum(torch.where(good_idx, torch.tensor(0., device='cpu'),
                                                       loss_order)) / unavailable_number if unavailable_number else torch.tensor(
                0., device='cpu')  # Changed from 'cuda'
            rank_weight = torch.where(good_idx, loss_order, unavailable_weight) / (n - 1)
            grads += torch.sum(batch_noise / sigma * (rank_weight.view((-1,) + (1,) * (len(batch_noise.size()) - 1))),
                               0)
        else:
        ...
        if count_in == 0:
            return None, None
        logging.info('count in: {}'.format(count_in))
        print(f"DEBUG: Running sampling step {count_in}, Loss total: {loss_total}")
...
def targeted_video_attack(vid_model, vid, target_vid, directions_generator, target_class,
                          ...
                          max_iter=10000, sigma=1e-5, sub_num_sample=12,
                          image_split=1):
    print(f"DEBUG: Running targeted_video_attack with max_iter={max_iter}, sigma={sigma}")
    num_iter = 0
    adv_vid = target_vid.clone()
    cur_eps = starting_eps
    ...
    while num_iter < max_iter:
        print(f"DEBUG: Iteration {num_iter}, Current Query Count: {num_iter}")
        top_val, top_idx, _ = vid_model(adv_vid[None, :])
        num_iter += 1

        tentative_directions = directions_generator(adv_vid).to('cpu')  # Changed from .cuda()
        group_gen.initialize(tentative_directions)
        ...
        if target_class == top_idx[0][0] and cur_eps <= eps:
            logging.info('early stop at iteration {}'.format(num_iter))
            return True, num_iter, adv_vid
...
def untargeted_video_attack(vid_model, vid, directions_generator, ori_class,
                            rank_transform=False, eps=0.05, max_lr=1e-2, min_lr=1e-3, sample_per_draw=50,
                            max_iter=10000, sigma=1e-5, sub_num_sample=12, image_split=1):
    print(f"DEBUG: Running untargeted_video_attack with max_iter={max_iter}, sigma={sigma}")
    
    num_iter = 0
    adv_vid = torch.clamp(vid.clone() + (torch.rand_like(vid) * 2 - 1) * eps, 0., 1.)
    cur_lr = max_lr  # Initialize learning rate
    stuck_loss_counter = 0  # Counter to track if loss is stuck
    ...
    while num_iter < max_iter:
        ...
        # Early stopping if the adversarial attack succeeds
        if ori_class != top_idx[0][0]:
            logging.info(f"Early stop at iteration {num_iter}. Attack successful!")
            return True, num_iter, adv_vid

        # Generate adversarial directions
        tentative_directions = directions_generator(adv_vid).to('cpu')  
        group_gen.initialize(tentative_directions)

        # Compute rectification vector
        l, g = sim_rectification_vector(vid_model, adv_vid, tentative_directions, sample_per_draw, sigma,
                                        ori_class, rank_transform, sub_num_sample, group_gen, untargeted=True)

        if l is None and g is None:
            logging.info('NES sim fails, trying again...')

            # Increase sigma if attack keeps failing
            stuck_loss_counter += 1
            if stuck_loss_counter >= 3:
                sigma *= 2  # Double the noise strength
                logging.info(f"WARNING: Increasing sigma to {sigma} due to repeated failures.")
                stuck_loss_counter = 0  # Reset counter

            continue

        # Adjust learning rate dynamically if loss is stuck
        if l == 0.0:
            stuck_loss_counter += 1
            if stuck_loss_counter >= 5:  # Reduce learning rate after 5 failed attempts
                cur_lr = max(cur_lr / 2, min_lr)
                logging.info(f"WARNING: Loss stuck at 0.0. Reducing learning rate to {cur_lr}")
                stuck_loss_counter = 0  # Reset counter
        else:
            stuck_loss_counter = 0  # Reset if loss changes

        # Apply perturbation
        rectified_directions = group_gen.apply_group_change(tentative_directions, torch.sign(g))
        num_iter += sample_per_draw  # Update iteration count

        adv_vid += cur_lr * rectified_directions
        adv_vid = torch.clamp(adv_vid, 0., 1.)  # Keep values within valid range

        logging.info(f"Step {num_iter}: Loss {l} | Learning Rate {cur_lr}")
