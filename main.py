...
def main():
    ...
    if multiple_gpus:
        ...
        i3d = nn.DataParallel(i3d).to('cpu')
    ...
    if 'resnet50' in args.image_models:
        ...
        resnet50_extractor = ResNetFeatureExtractor(resnet50, layer).eval().to('cpu')
        if multiple_gpus:
...
def main():
    ...
    image_split = args.image_split
    sub_num_sample = args.sub_num_sample
    gpus = args.gpus  # No longer needed since we are using CPU

    multiple_gpus = len(gpus) > 1

    logging.basicConfig(
    stream=sys.stdout,
    level=logging.DEBUG,  # Set this to DEBUG to ensure all logs are printed
    ...
    # Model Initialization
    i3d = InceptionI3d(400, in_channels=3)
    i3d.load_state_dict(torch.load('inception_i3d/models/rgb_imagenet.pt', map_location=torch.device('cpu')))
    i3d.to('cpu')  # Removed .cuda()
    i3d.eval()
    
    if multiple_gpus:
        i3d = nn.DataParallel(i3d).to('cpu')  # Updated for CPU usage
    ...
    if 'resnet50' in args.image_models:
        resnet50 = models.resnet50(pretrained=True)
        resnet50_extractor = ResNetFeatureExtractor(resnet50, layer).eval().to('cpu')  # Changed from .cuda()
        if multiple_gpus:
            resnet50_extractor = nn.DataParallel(resnet50_extractor, advs_device).eval().to('cpu')  # Changed from .cuda()
        extractors.append(resnet50_extractor)

    if 'densenet121' in args.image_models:
        densenet121 = models.densenet121(pretrained=True).eval()
        densenet121_extractor = DensenetFeatureExtractor(densenet121, layer).eval().to('cpu')  # Changed from .cuda()
        if multiple_gpus:
            densenet121_extractor = nn.DataParallel(densenet121_extractor, advs_device).eval().to('cpu')  # Changed from .cuda()
        extractors.append(densenet121_extractor)

    if 'densenet169' in args.image_models:
        densenet169 = models.densenet169(pretrained=True).eval()
        densenet169_extractor = DensenetFeatureExtractor(densenet169, layer).eval().to('cpu')  # Changed from .cuda()
        if multiple_gpus:
            densenet169_extractor = nn.DataParallel(densenet169_extractor, advs_device).eval().to('cpu')  # Changed from .cuda()
        extractors.append(densenet169_extractor)

    directions_generator = TentativePerturbationGenerator(
        extractors, part_size=32, preprocess=need_preprocess, device='cpu'  # Changed from advs_device[0]
    )

    # Attack...
    vid = np.load(args.video)
    vid = torch.tensor(vid, dtype=torch.float, device='cpu')  # Changed from 'cuda'
    if vid.shape[-1] == 3:
        vid = vid.permute(0, 3, 1, 2)
    vid_label = args.label

    # Load Target Video (if applicable)
    target_vid = np.load(args.target_video)
    target_vid = torch.tensor(target_vid, dtype=torch.float, device='cpu')  # Changed from 'cuda'
    target_label = args.target_label


        # Check if the video is already misclassified
    top_val, top_idx, _ = vid_model(vid[None, :])
    print(f"DEBUG: Original classification - Predicted Label: {top_idx.item()}, Confidence: {top_val.item()}")

    if top_idx.item() != vid_label:
        print("WARNING: The model already misclassifies the video! The attack may not work.")

    # Run Attack
    if not untargeted:
        directions_generator.set_targeted_params(target_vid.to('cpu'), random_mask)  # Changed from .cuda()
        res, iter_num, adv_vid = targeted_video_attack(
        ...
        directions_generator.set_untargeted_params(vid.to('cpu'), random_mask, scale=5.)  # Changed from .cuda()
        res, iter_num, adv_vid = untargeted_video_attack(
    ...
    if res:
        if untargeted:
            logging.info(
                '--------------------untargeted attack succeed using {} queries-----------------------'.format(iter_num))
        else:
            logging.info(
                '--------------------{} transfer to {} using {} queries-----------------------'.format(
                    vid_label, target_label, iter_num
                )
            )
    else:
        logging.info('--------------------Attack Fails-----------------------')
